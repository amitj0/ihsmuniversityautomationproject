package com.ihsm.university.pageobjects.documents;
...
public class Documents_VisaInfo_OffVisa extends BasePage {
	...
	@FindBy(xpath = "(//div[@id='STUDENTVISA']//label[contains(text(),'Visa Type')]/following-sibling::div[1]//ng-select[@name='VISATYPE'])[1]")
	...
	public void selectAddress(String addressType) {
		...
		for (WebElement option : selectAddFieldList) {
			if (option.getText().equalsIgnoreCase(addressType)) {
				blinkElement(option);
				safeClick(option);
				return;
			}
	...
	public Documents_VisaInfo_OnVisa fillVisaInfoOffVisaForm(String visaType, String addressType, String currentVisaExpDate,
			String nextVisaIssDate, String nextVisaExpDate, String expiryDateNotary, String visaNumber,
			String currentRegAdd, String comments,  String filePath) {
		clickVisaInfoTab();
		clickOfflineVisaTab();
